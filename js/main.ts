...
// Initializes the Milestone structure so that we can correctly parse the data.
// This looks similar to the JSON structure
interface Milestone {
    milestoneType: {
...
// Initializes the Stripe Segment so that we can read the data in a more sensible way.
...
// Initialises the Patient Data structure so that we can parse the data about the patient E.G. name and birthday.
interface Patient {
    firstName: String,
    lastName: String,
    birthDate: Date,
...
function AddToHTML(personData) {
    ...
    patient.milestones.forEach(element => {
        ...
        let segmentToAdd: StripeSegment = {
...
interface NigelStripeSegment {
    col: String,
    len: Number,
    txt: String
}

interface NigelDataFormat {
    name: String,
    referral: Date,
    stripe: [NigelStripeSegment]
}

const NigelData = [{
    "name" : "James Blunt",
    "referral" : "2021-11-10",
    "stripe": [{"col" : "red","len": 40, "txt":"Triage"},//Triage CT thorax requested
    {"col" : "grn","len": 80, "txt":"Td"},//CT thorax done
    {"col" : "red","len": 10, "txt":"Cr"},//Clinic Requested
    {"col" : "grn","len": 10, "txt":"Cd"},//Clinic done, PET-CT requested
    {"col" : "grn","len": 170, "txt":"PET done"},//PET-CT done
    {"col" : "red","len": 10, "txt":"Er"},//EBUS requested
    {"col" : "grn","len": 10, "txt":"Ed"},//EBUS done, MDT requested
    {"col" : "grn","len": 60, "txt":"MDT done, Surgery requested"},//MDT done, Surgery requested
    {"col" : "grn","len": 160, "txt":"Surgery"},//surgery
    {"col" : "white","len": 70, "txt":""}]
    }
];

function NigelAddToHTML(personData) {
    const patient: NigelDataFormat =  {
